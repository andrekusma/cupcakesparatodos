...
export async function store(req, res, next) {
  try {
    const { nome, descricao = '', preco, preco_cents, estoque = 0, image_url } = req.body;
    ...
    if (cents == null || isNaN(cents) || cents < 0) {
      return res.status(400).json({ message: 'Preço inválido. Envie preco (ex: 12.34) ou preco_cents (centavos).' });
    }
